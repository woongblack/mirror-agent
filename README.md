...
| Phase 2: Re-Applicator v0.1 | 🔄 진행 중 | Analyzer/Matcher/Generator/Defender/Scorer/Reporter **stub 완료, 로직 미구현** |
| Phase 3: 실사용 + 개선 | ⏸ 대기 | Phase 2 완료 후 |

### 다음 작업

각 stub 모듈(`src/mirror_agent/*.py`)의 `raise NotImplementedError` 부분을 채우는 것.
구현 순서: `llm.py` → `analyzer.py` → `matcher.py` → `generator.py` → `defender.py` → `scorer.py` → `reporter.py`

검증: `eval/ground_truth.json`의 3건 중 2건 이상 재현 + `rule_supplier_first` Critical Hit 필수.
